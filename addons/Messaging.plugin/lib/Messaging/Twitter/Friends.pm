package Messaging::Twitter::Friends;
...
sub destroy {
    ...
    my $foll = MT->model('tw_follower')->load(
        {
...
sub show {
    ...
    foreach my $var (qw( source target )) {
        ...
        if ( $params->{ $var . '_screen_name' } ) {
            my $user =
              MT->model('author')
              ->load( { name => $params->{ $var . '_screen_name' } } );
            unless ($user) {
                return $app->error( 404,
                        "User ($var)"
                      . $params->{ $var . '_screen_name' }
                      . ' not found.' );
            }
            $$var = $user->id;
        }
    ...
    unless ($source_user) {
        return $app->error( 404, "Source user not found." );
    }
    my $target_user = MT->model('author')->load($target);
    unless ($target_user) {
        return $app->error( 404, "Target user not found." );
    }
